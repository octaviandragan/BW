...
test('Verify last name does not accept numbers', async ({ page }) => {
    const userProfile = new UserProfilePage(page);
    await page.goto('https://qa-assessment.pages.dev/');
    console.log(`Testing form submission with an invalid last name`);
    // Fill the form with an invalid first name (containing numbers)
    await fillUserProfile(userProfile, { lastName: "Smith123" });
    //Click submit
